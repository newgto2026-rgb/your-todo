...
| Friend Todo Visibility | 친구가 허용한 할 일을 읽기 전용 `친구 할일`로 확인 |
| AI Todo Draft | 한국어 자연어 입력을 편집 가능한 Todo 초안으로 변환 |
| Profile/Auth | 계정 상태, 알림, push token, 로그아웃 흐름 관리 |

앱의 핵심은 여러 화면이 같은 task 현실을 바라보도록 만드는 것이다. 개인 Todo와 Assigned Todo는 domain 계층에서 합성되고, 친구가 허용한 Observed Todo는 별도 읽기 전용 흐름으로 유지된다. Todo, Calendar, Widget, Friends 화면은 각 데이터의 의미를 보존한 채 UI 모델로 변환해 표현한다.

### 사용자 매뉴얼

[사용자 매뉴얼 PDF](docs/YourTodo_User_Manual.pdf)는 YourTodo가 어떤 앱인지 먼저 이해할 수 있도록 앱 개요와 함께 로그인, 프로필 설정, 개인 할 일, 친구, 공유 할 일, AI 할 일 등록, 캘린더, 홈 화면 위젯까지 실제 화면 캡처와 단계별 조작 흐름을 정리한 사용 가이드다.
...
- person-level visibility grant와 read-only observed todo 흐름 설계
