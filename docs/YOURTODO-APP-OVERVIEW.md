...
YourTodo는 개인 할 일 관리, 친구와의 할 일 공유, 캘린더 기반 일정 확인, 홈 화면 위젯, AI 초안 생성을 하나의 Android 앱 안에 통합한 생산성 앱이다.

핵심은 단순한 Todo 목록이 아니라 "내가 해야 할 일"과 "친구에게 받은 일"을 같은 사용자 표면에서 자연스럽게 다루는 것이다. 사용자는 빠르게 개인 Todo를 만들고, 친구에게 할 일을 보내고, 자동수락 관계에서는 수락 절차 없이 할 일이 바로 목록과 캘린더에 반영되는 협업 경험을 얻는다.
...
| Calendar | 날짜별 indicator, 선택일 할 일 |
| Widget | 월간 count/chip |
...
| Room | Todo, Category, Reminder, Assignment cache |
| DataStore | Auth session, sync cursor, UI preference, push token, freshness |
| Retrofit/OkHttp | Auth, Todo sync, Friends, Assignments, Push, AI proxy |
...
| 테스트 영역 | 예시 |
|---|---|
| Domain use case | TaskSurface, WorkspaceRefresh, Assignment use cases |
| Data repository | Todo sync, Assignment cache, Auth cleanup, Reminder |
...
| 협업형 Todo 모델 | 친구 요청, shared todo, direct assignment를 자연스럽게 연결 |
| 일관된 task surface | Todo, Calendar, Widget이 같은 domain 정책을 공유 |
